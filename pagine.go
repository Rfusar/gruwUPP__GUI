...
func paginaSegreta1(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		cmd := exec.Command("cmd", "/c", "cd C:\\Users\\Utente\\Desktop\\proveProgetti\\prove_Go\\provaServer\\static\\strumenti\\python && .\\venv\\Scripts\\activate && python zip.py \\.env.7z")
...
func paginaSegreta2(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		cmd := exec.Command("cmd", "/c", "cd C:\\Users\\Utente\\Desktop\\proveProgetti\\prove_Go\\provaServer\\static\\strumenti\\python && .\\venv\\Scripts\\activate && python zip.py \\segreto.7z")
		output, err := cmd.CombinedOutput()
		if err != nil {
			fmt.Println("Errore durante l'esecuzione del comando:", err)
			done <- false
